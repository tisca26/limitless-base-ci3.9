...
class Users extends Privy
{
    ...
    public function index()
    {
        ...
        $template['_B'] = 'users/users_index';
        $template['_S'] = 'users/users_index';
    ...
    public function insertar()
    {
        ...
        $template['_B'] = 'users/users_insertar';
        $template['_S'] = 'users/users_insertar';
        $this->load->template_view($this->plantilla, $data, $template);
    }
    ...
    public function editar($id = 0)
    {
        ...
        $template['_B'] = 'users/users_editar';
        $template['_S'] = 'users/users_editar';
        $this->load->template_view($this->plantilla, $data, $template);
    }
    ...
    public function frm_editar()
    {
        ...
        $this->form_validation->set_rules('apellido_paterno', 'Apellido Paterno', 'required|min_length[2]');
        $this->form_validation->set_rules('apellido_materno', 'Apellido Materno', 'required|min_length[2]');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $usuario = $this->input->post();
            $grupos = $usuario['groups'];
            log_message('debug', 'grupos: ' . implode(',', $grupos));
            unset($usuario['groups']);
            unset($usuario['repasswd']);
            $usuario['estatus'] = isset($usuario['estatus']) ? 1 : 0;
            if ($usuario['passwd'] !== '') {
